...
A very fast 2D **concave hull** algorithm by distance implemented in
Python.
...
All edge lengths exceeding `tol` parameter will be attempted to remove.
This parameter’s unit will thus be the same as your coordinates. Wether
it is meters, feet or degrees.
...
If there is an uncertianty on which value to use for `to` the `estimate`
function can be used to provide an estimate. The default percentile
`perc` is 92.5%. At this value only outside border edges longer than
92.5% of all the edges will be attempted to be removed.
...
Possible features in the future depending on feedback and/or special
requests.

- Output to `PNG` for quick visualization
- Support and elevation (Z value) on the polygon vertices
- Support for holes
